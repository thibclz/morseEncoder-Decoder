...
typedef struct wavfile_header_s //header of the WAV file
{
    char    ChunkID[4];
    int32_t ChunkSize;
    char    Format[4];
...
void write_PCM16_stereo_header(FILE* file_p, int32_t SampleRate, int32_t FrameCount)
{
    int ret;
    ...
    fwrite(&wav_header, sizeof(wavfile_header_t), 1, file_p); //write the value of wav_header in file_p in one block of sizeof(...) size
}
